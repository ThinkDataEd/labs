...
- **(1) Start by loading the `titanic` data. Write and run code calculating the `mean` `age` of people in the data but `shuffle` their `survival` status 2000 times.**
...
- Using the `titanic` data:
    - **(12) Were women on the Titanic typically younger than men? Write and run code using a histogram, 2000 random shuffles and a normal model to answer the question.**
    
    
- Using the `cdc` data:
    - **(13) Using 2000 random shuffles and a normal model, how much taller would the typical male have to be than the typical female in order for the difference to be in the upper 1% by chance alone?**
