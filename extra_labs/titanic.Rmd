...
The output shows that on average, survivors paid about 27 more pounds than non-survivors, but is this enough evidence to say that those that paid a higher fare were more likely to survive?
...
```{r, eval = FALSE, echo = TRUE}
shuffled_survived <- do(2000) * mean(~fare | shuffle(survived), data = titanic)
```
...
histogram(~diff, data = shuffled_survived, 
          xlim=c(-30,30), 
          main="Shuffled Differences in Fare between Survivors and Non-Survivors")
```
