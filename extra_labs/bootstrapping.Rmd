...
**Key Concept:** A bootstrap sample is a random sample taken WITH replacement from our original data, using the same sample size. We can then calculate a statistic from this random sample and create a bootstrap distribution by doing this many times. Once we have this distribution, we can use percentiles to find a confidence interval.
...
Now that we have which observations to sample, we’ll use `slice()` to use these rows to create a new set of data.
```{r, echo = TRUE, eval = FALSE}
