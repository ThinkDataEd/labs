...
- <span class="blue">`View` your `cdc` data and find the columns for `height` and `sex` (use the _History_ pane again if you need help to `View` your data).</span>
    ...
    - **Is `sex` a numerical or categorical variable? Why?**
    - **List either the different categories or what you think the measured units are for `height` and `sex`.**
...
- <span class="blue">Just like we did for the `histogram`, is it possible to create a _split_ `bargraph`? Try to create a `bargraph` of `drive_text` that's split by `sex` to find out.</span>
