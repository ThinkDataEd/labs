...
- <span class="blue">Now that we've got the data loaded, we need to clean it to get it ready for use _(Look at lab 1F for help)_. Specifically:</span>
    - We want to name the variables: `"age"`, `"lung_cap"`, `"height"`, `"sex"`,`"smoker"`, in that order.
    - Change the type of variable for `sex` and `smoker` from _numeric_ to _character_.
- <span class="blue">After changing the variable types for `sex` and `smoker`:</span>
    - For `sex`, use `recode` to change `"1"` to `"Male"` and `"0"` to `"Female"`.
